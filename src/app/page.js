...
export default function Home() {
  ...
  return (
    ...
    >
          ...
          {messages.map((msg, idx) => (
            <motion.div
              ...
              className={`p-3 my-2 rounded-lg max-w-[80%] ${
                msg.role === "user" ? "bg-red-600 self-end ml-auto rounded-xl" : "bg-gray-700 self-start rounded-xl"
              }`}
      ...
      <Card className={`${hasSentMessage ? "mt-80" : "mt-40"} bg-white-800 transition-all duration-300` }>
        <CardContent className="p-4 flex gap-3 border-none">
          ...
          />
          <Button onClick={sendMessage} disabled={loading} className="bg-red-500 hover:bg-blue-600 rounded-xxl">
            {loading ? "..." : "Send"}
          </Button>
        </CardContent>
      </Card>
